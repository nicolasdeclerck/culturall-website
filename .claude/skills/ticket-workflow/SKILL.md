...
description: Workflow complet de traitement d'un ticket GitHub pour culturall-website, de l'analyse initiale jusqu'à l'approbation finale. Gère N cycles de review et corrections via un fichier d'état local (.claude-state.json). Déclencher sur le label "analyze" posé par un humain. Inclut : analyse, plan technique, développement, tests unitaires, PR, code review, tests browser via agent-browser, corrections post-review et post-tests-browser. Un seul label au démarrage (in progress), un seul à la fin (approved).
...
> **Placeholders à remplacer lors de la mise en place :**
> - `nicolasdeclerck/culturall-website` : ton repo GitHub (ex. `myorg/myproject`)
> - `/workspace/culturall-website` : chemin du clone interne au container worker (ex. `/workspace/myproject`)
...
```bash
BRANCH_NAME=$(git -C /workspace/culturall-website branch -r \
  | grep "origin/feat/issue-{ISSUE_NUMBER}-" | head -1 | xargs | sed 's|origin/||')

WORKTREE_PATH="/workspace/culturall-website-issue-{ISSUE_NUMBER}"

if [ -n "$BRANCH_NAME" ]; then
  git -C /workspace/culturall-website fetch origin
  git -C /workspace/culturall-website worktree add "$WORKTREE_PATH" "$BRANCH_NAME" 2>/dev/null \
    || git -C /workspace/culturall-website worktree add "$WORKTREE_PATH" \
       --track -b "$BRANCH_NAME" "origin/$BRANCH_NAME" 2>/dev/null \
    || echo "Worktree déjà existant sur $BRANCH_NAME, réutilisation"
else
  git -C /workspace/culturall-website fetch origin
  git -C /workspace/culturall-website worktree add "$WORKTREE_PATH" origin/main 2>/dev/null \
