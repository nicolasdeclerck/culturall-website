...
- Identifie les modules / apps concernés par le besoin
- Examine les fichiers (modèles, vues, routes, composants…) liés au besoin
- Repère les patterns existants (factories de test, conventions de nommage, etc.)
- Vérifie les dépendances installées (gestionnaire de paquets du projet)

> Adapte la liste ci-dessus à ton stack : pour un projet Django ce sera
> `apps/`, `models.py`, `api_views.py` ; pour un Node monorepo ce sera
> `packages/`, `services/`, `package.json` ; etc.
