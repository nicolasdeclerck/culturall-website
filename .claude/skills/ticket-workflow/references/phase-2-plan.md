...
- Migrations / changements de schéma : toujours une tâche indépendante
- Tests : tâche séparée de l'implémentation
- Ordre logique : modèle/schéma → couche métier → couche présentation → tests
- Principe YAGNI : signaler tout choix dépassant les consignes

> Adapte l'ordre des tâches au stack du projet (front, back, full-stack…).
...
{
  "tasks": [
    {"index": 0, "description": "Créer le modèle X dans <chemin>", "status": "pending"},
    {"index": 1, "description": "Ajouter la migration", "status": "pending"},
    {"index": 2, "description": "Créer la vue API dans <chemin>", "status": "pending"},
    {"index": 3, "description": "Écrire les tests dans <chemin>", "status": "pending"}
  ]
}
```
