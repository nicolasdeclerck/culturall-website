...
> - `/workspace/culturall-website` : chemin du clone interne au container worker
> - `testuser@example.com` / `Testpass123!` : credentials de l'utilisateur de test seedé par `tnr-docker.sh`
...
| `BASE_URL` | URL nginx de l'env éphémère | `http://culturall-website-nginx-1:80` |
| `API_URL` | URL API de l'env éphémère | `http://culturall-website-nginx-1:80` |
...
STATE_FILE="/workspace/culturall-website-issue-${ISSUE_NUM}/.claude-state.json"
...
RUN_URL="${GITHUB_SERVER_URL:-https://github.com}/${GITHUB_REPOSITORY:-nicolasdeclerck/culturall-website}/actions/runs/${GITHUB_RUN_ID:-}"
```
