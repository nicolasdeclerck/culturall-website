...
> - `https://culturall-website.nickorp.com` : URL de production (ex. `https://app.example.com`)
> - `testuser@example.com` / `Testpass123!` : credentials d'un utilisateur de test
> - `/admin/login/` : chemin de la page de connexion (ex. `/login`)
...
```bash
BASE_URL="${BASE_URL:-https://culturall-website.nickorp.com}"
API_URL="${API_URL:-https://culturall-website.nickorp.com}"
```

Toutes les URLs utilisées dans les tests sont construites à partir de
`BASE_URL` (ex : `${BASE_URL}/admin/login/`).
...
fidèlement la production mais sans Traefik ni certificat TLS.
...
TRACKING_ISSUE=$(gh issue create \
  --repo nicolasdeclerck/culturall-website \
...
agent-browser --session user1 open ${BASE_URL}/admin/login/
...
agent-browser --session user2 open ${BASE_URL}/admin/login/
# ... même flow avec un second compte de test ...
```
...
```bash
gh search issues "regression {test_id}" --repo nicolasdeclerck/culturall-website --state open --json title --jq '.[].title'
```
...
7. **Les tests [OWNER]** nécessitent de créer des données de test en début de session si elles n'existent pas
